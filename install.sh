...
Type=simple
LimitNOFILE=1000000
ExecStart=/opt/gohttp/gohttpserver -r ./public --port 8000 --auth-type http --auth-http birdza:${admintoken}
...
EOF
    ...
    echo -e "Username is birdza and password is ${admintoken}"
    if [[ -z "$http" ]]; then
